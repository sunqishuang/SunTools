...
////    //订单金额,单位（分）
//    NSString *order_price   = @"100";//1分钱测试
    
 
//    [NSThread sleepForTimeInterval:2];
    
    //订单标题，展示给用户
//    NSString *order_name    = [NSString stringWithFormat:@"%@",self.order_name];
    //订单金额,单位（分）
//    NSString *order_price   = [NSString stringWithFormat:@"%@",self.order_price];//1分钱测试
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: self.notify_URL        forKey:@"notify_url"];          //支付结果异步通知
    [packageParams setObject: self.order_no           forKey:@"out_trade_no"];        //商户订单号
